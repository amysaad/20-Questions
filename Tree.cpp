...
void Tree::DeleteAll(Node *pNode)
{
    if (pNode!=0)
    {
        DeleteAll(pNode->Node::LChild());
        DeleteAll(pNode->Node::RChild());
        DeleteAll(pNode);   //  would you do DeleteAll(root) ?
    }
...
//POST: entire tree saved to a file
{
    ...
    {
        cout << "Hmm... the file called" << outputFileName
             << " doesn't work " << endl << "Sorry!" << endl;
        // system("PAUSE");
        exit(-1);
    } // if
...
//POST: entire tree loaded from a file
{
    ...
    {
        cout << "Hmm... the file name " << inputFileName
             << " didn't open " << endl << "Sorry!" << endl;
        // system("PAUSE");
        exit(-1);
    } // if
